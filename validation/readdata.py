"""
Module with classes to create pyspark and pandas dataframe from data in CSV, XLSX, JSON, TXT & Parquet format. 
"""
import numpy as np
import pandas as pd
...
class ReadDataPyspark:
    ...
    def read_file_format(self):
        """
        Method to identify file format (CSV, JSON, TXT or Parquet) and invoke respective function to create 
    ...
    def read_json_data(self):
        ...
        df = JsonToDataframe(self.filepath)
...
class ReadDataPandas: 
    """
    Class to create pandas dataframe from data in CSV, XLSX, JSON, TXT and Parquet form. 
    """
    ...
    def read_csv_data(self):
        ...
        usecols = ['Attribute_Name', 'Data_Type', 'Nullable','Data_Structure', 'Lookup_Table_Name',
                'Enhance_Table_Name', 'IS_PCI','IS_PII', 'IS_CPNI', 'Description']
        df = pd.read_csv(self.filepath, usecols=usecols)
        df['Data_Type_Limit'] = df['Data_Type'].copy().apply(lambda x: x[x.find('(')+1:x.find(')')] if x.count('(')>0 else np.nan)
