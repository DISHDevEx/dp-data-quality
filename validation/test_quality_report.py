...
def test_column_validation_result(qr, request, datatype, function,
    ...
    assert isinstance(qr.column_validation_results(datatype_df, request.getfixturevalue(function)), pd.DataFrame),\
    'Method does not return dataframe with valid datatype function as input.' 

def test_column_validation_result(qr,
                    datatype_dictionary, dataframe_with_row_id):
    """
    Method to test that column_validation_results method returns a
    ...
    assert isinstance(qr.column_validation_results('numeric', 'foo-bar'), type(None)),\
