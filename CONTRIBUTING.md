# Contributing to Data Quality Module
...
### Adding Your Rule Check to The Directory Hierarchy
Adding your data quality rule check to the _dp-data-quality_ library is a relatively easy process; however, it is important that the steps below are followed closely to ensure the new rule check fits in correctly with the existing framework.  To make things easy to follow, an example algorithm ___circles___ has been included as an example to follow.

#### 1. Validation Sub-Directory in [_dp-data-quality_](https://github.com/DISHDevEx/dp-data-quality/tree/main/validation) Directory
Navigate to the _validation_ sub-directory within the _dp-data-quality_ repository:
```console
> dp-data-quality
    > validation
        __init__.py
    ...
    > [OtherPackages]
```
#### 2. Add a New Generic Rule Check Under the [_data_rulebook_](/data_rulebook.py/) File
Once in validation sub-directory, add new generic rule check to __GenericRulebook__ class in [__validation_rulebook.py__](https://github.com/DISHDevEx/dp-data-quality/blob/main/validation/data_validation.py) file.  For a new function named generic_funtion, the validation subdirecty will be updated as follows - 
```console
> dp-data-quality
    > validation
        __init__.py
        ...
        data_rulebook.py
            ...
            class DatatypeRulebook(GenericRuleBook):
                def datatype_validation_functions:
                    """
                    Update dictionary with new generic_function
                    """
                    pass
        quality_report.py
    > [OtherPackages]
```
#### 3. Add a New Generic Rule Check in the [_devex_sdk_](/data_rulebook.py/) Sub-Directory
In, create a new directory.  The directory should be named with your algorithm name, the same name you included in the branch you checked out above.  For the _circles_ sub-package, the new directory would be titled as follows:
```console
> dp-data-quality
    > validation
        __init__.py
        ...
        data_rulebook.py
            class DatatypeRulebook(GenericRuleBook):
                 def datatype_function():
                    pass
                def datatype_validation_functions:
                    """
                    Update dictionary with new datatype_function
                    """
                    pass
        quality_report.py
    > [OtherPackages]
...
#### 4. Add a New Business Specific Rule Check in the [_devex_sdk_](/data_rulebook.py/) File
In, create a new directory.  The directory should be named with your algorithm name, the same name you included in the branch you checked out above.  For the _circles_ sub-package, the new directory would be titled as follows:
```console
> dp-data-quality
    > validation
        ...
        data_rulebook.py
            class BusinessUsecaseRulebook(GenericRulebook):
                def business_usecase1():
                    pass
                def business_usecase2():
                    pass
