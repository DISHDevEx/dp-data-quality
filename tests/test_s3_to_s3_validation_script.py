...
def test_prefix_to_list_correct(s3_bucket, s3_prefix, s3_resource, request):
    ...
    """
    expected_result = ['test/', 'test/fake_file.csv', 'test/hello.py',
                       'test/s3_to_s3_validation.csv',
                       'test/s3_to_s3_validation_second.csv', 'test/spark_setup.py']
...
def test_rename_columns_incorrect_df(pyspark_df, renames, request):
    """
    Test function get_sns_name with incorrect df input:
        pyspark_df
        renames
    Pass criteria:
        renamed_df is None
    """
    if pyspark_df == "fixture_second_df":
        pyspark_df = request.getfixturevalue(pyspark_df)
    renamed_df = rename_columns(pyspark_df, **renames)
...
@pytest.mark.parametrize(
    ["pyspark_df","renames"],
    [
        ("fixture_second_df", {"fake_size": "b_size","path": "b_path"})
    ]
)
def test_rename_columns_incorrect_dict(pyspark_df, renames, request):
    """
    Test function get_sns_name with incorrect dict input:
        pyspark_df
        renames
    Pass criteria:
        renamed_df is pyspark_df
    """
    pyspark_df = request.getfixturevalue(pyspark_df)
    renamed_df = rename_columns(pyspark_df, **renames)
    pyspark_df = pyspark_df.withColumnRenamed("path", "b_path")
    assert renamed_df.schema == pyspark_df.schema
    assert renamed_df.columns == pyspark_df.columns
    assert renamed_df.select('b_path').rdd.flatMap(lambda x: x).collect() ==\
        pyspark_df.select('b_path').rdd.flatMap(lambda x: x).collect()
    assert renamed_df.select('size').rdd.flatMap(lambda x: x).collect() ==\
        pyspark_df.select('size').rdd.flatMap(lambda x: x).collect()
    assert renamed_df.select('date').rdd.flatMap(lambda x: x).collect() ==\
        pyspark_df.select('date').rdd.flatMap(lambda x: x).collect()
